    ...
    in
    {
      packages = forEachSystem (
          ...
          specCatalog =
            pkgs.runCommand "spec-catalog"
              ...
              ''
                ...
                export HOME="$TMPDIR"
                cd ${governance}
                python3 ${governance}/tools/cue-records-gate.py \
                  --root ${governance} \
        ...
        // rec {
          ...
          ops-bootstrap-placeholder = pkgs.runCommand "ops-bootstrap-placeholder" { } ''
          ...
          # No packages.default: experimental bootstrap placeholder is explicit only.
        }
        // cdp.packages
      );
        ...
        // {
          ...
          ops-cdp-core =
            pkgs.runCommand "ops-cdp-core-check"
              ...
              '';
          ops-bootstrap-placeholder = pkgs.runCommand "ops-bootstrap-placeholder-check" { } ''
            test -e ${self.packages.${pkgs.stdenv.hostPlatform.system}.ops-bootstrap-placeholder}/share/ops/README
            touch $out
          '';
        }
      );
