...
def source_put_result(args: argparse.Namespace, result: dict[str, Any]) -> dict[str, Any]:
    ...
    if upload_log.is_file():
        ...
        parsed = low["json"]
    if upload_command == "chromium-cdp-upload-project-source-text":
        upload_text_tail = str(parsed.get("upload", {}).get("textTail", "")) if isinstance(parsed, dict) else ""
        visible = bool(parsed and parsed.get("upload") and parsed["upload"].get("ok") and file_path.name in upload_text_tail)
    else:
    ...
    result.update({
        ...
        "verificationLevel": "visible-only" if visible else "none",
        "uploadResult": parsed,
    })
    return result
