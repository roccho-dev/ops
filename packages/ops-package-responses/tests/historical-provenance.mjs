...
const gitPasses = (...args) => spawnSync("git", ["-C", repoRoot, ...args], { stdio: "ignore" }).status === 0;
...
const currentTree = gitText("rev-parse", "HEAD^{tree}");
assert.equal(
...
const targetToImplementation = gitPasses(
  "merge-base", "--is-ancestor", manifest.target_commit, receipt.implementation_commit,
);
const implementationToCurrentHead = gitPasses(
  "merge-base", "--is-ancestor", receipt.implementation_commit, currentHead,
);
assert.equal(targetToImplementation, true, "target commit is not an ancestor of implementation commit");
assert.equal(implementationToCurrentHead, true, "implementation commit is not an ancestor of current head");
...
process.stdout.write(`${JSON.stringify({
  authority: false,
  kind: "ops.packageObligationGoldenGitProvenance.v2",
  status: "PASS",
  ...
  currentTree,
  ancestry: {
    targetToImplementation,
    implementationToCurrentHead,
  },
  signatureVerification: "NOT_PERFORMED_LOCAL; GitHub provider evidence is separate",
})}\n`);
