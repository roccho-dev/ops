...
{
  const result = promoteProposalToModelQueue({ ...proposal, status: 'promoted' }, {
    confirm: true,
    confirmedBy: 'human-review',
    proposalDigest: proposalDigest({ ...proposal, status: 'promoted' }),
  });
  assert.equal(result.ok, false);
...
{
  const result = promoteProposalToModelQueue({ ...proposal, acceptedLedger: true }, {
    confirm: true,
    confirmedBy: 'human-review',
    proposalDigest: proposalDigest({ ...proposal, acceptedLedger: true }),
  });
  assert.equal(result.ok, false);
