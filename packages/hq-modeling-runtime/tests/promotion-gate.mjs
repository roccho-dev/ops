...

{
  ...
  const result = promoteProposalToModelQueue(promoted, {
    confirm: true,
    confirmedBy: 'human-review',
    proposalDigest: proposalDigest(promoted),
  });
  assert.equal(result.ok, false);
...

{
  ...
  const result = promoteProposalToModelQueue(authority, {
    confirm: true,
    confirmedBy: 'human-review',
    proposalDigest: proposalDigest(authority),
  });
  assert.equal(result.ok, false);
