...
assert.deepEqual(runtimeBoundary.implementedNow, ['package-boundary-metadata']);

for (const issue of ['ops#40', 'ops#41', 'ops#42', 'ops#43', 'ops#44']) {
  assert.ok(
...
assert.match(runtimeBoundary.authorityBoundary.acceptedLedger, /not implemented in scaffold/);
...
assert.equal(summary.laterIssueCount, 5);
assert.deepEqual(summary.implementedNow, ['package-boundary-metadata']);
