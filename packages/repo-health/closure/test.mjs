...
const base = {
  ...
  conditions: [
    ...
    { id: 'c05', from: 'Effect', to: 'Readback', criterion: 'The actual effect is independently observed after execution.', evidence: ['readback references the effected target'] },
    { id: 'c06', from: 'Readback', to: 'Event', criterion: 'The observed result becomes evidence for the next business event or decision.', evidence: ['next event references the readback result'] },
  ],
};
...
const ask = async (state, questions) => {
  calls++;
  assert.equal(state.conditions.length, 6);
  assert.deepEqual(state.conditions.map((c) => c.id), ['c01', 'c02', 'c03', 'c04', 'c05', 'c06']);
...
assert.deepEqual(result.coverage, {
  ...
  claim: 'declared-closed-set',
});
assert.deepEqual(result.findings.map((f) => f.id), ['c06', 'c05', 'c04', 'c03', 'c02', 'c01']);
assert.ok(result.findings.every((f) => Array.isArray(f.subject) && f.subject[0] === 'edge'));
...
assert.equal(open.coverage.claim, 'examined-scope-only');
