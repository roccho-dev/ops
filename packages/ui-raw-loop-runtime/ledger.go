...
var (
	ErrConflict      = errors.New("semantic intent event_id conflict")
	ErrCorruptLedger = errors.New("corrupt authoring-intent ledger")
)

// AcceptedRecord is one immutable authoring-intent.jsonl row.
...
// The runtime has one active writer. This mutex covers concurrent goroutines;
// flock also rejects inter-process interleaving on the Linux deployment target.
var appendMutex sync.Mutex

// Append validates and durably appends one accepted intent before any external
// effect. Existing malformed, non-canonical, or torn history fails closed.
func (ledger Ledger) Append(intent Intent) (AppendResult, error) {
	...
	defer syscall.Flock(int(file.Fd()), syscall.LOCK_UN) // best-effort unlock on close
	...
	for _, record := range records {
		if record.Intent.EventID != intent.EventID {
			continue
		}
		if record.Digest == digest {
			return AppendResult{Status: AppendStatusNoChange, EventID: intent.EventID, Digest: digest}, nil
		}
		return AppendResult{}, fmt.Errorf("%w: event_id %q already has digest %s", ErrConflict, intent.EventID, record.Digest)
	}
	...
	return AppendResult{Status: AppendStatusAppended, EventID: intent.EventID, Digest: digest}, nil
}

// Read verifies every complete row and returns immutable accepted records.
...
func readAcceptedRecords(reader io.ReadSeeker) ([]AcceptedRecord, error) {
	...
	for index, line := range lines {
		...
		if err != nil {
			return nil, fmt.Errorf("%w: line %d cannot be canonicalized: %v", ErrCorruptLedger, index+1, err)
		}
		if !bytes.Equal(line, canonical) {
			return nil, fmt.Errorf("%w: line %d is not canonical", ErrCorruptLedger, index+1)
		}
		if prior, exists := seen[record.Intent.EventID]; exists {
			return nil, fmt.Errorf("%w: duplicate event_id %q (%s, %s)", ErrCorruptLedger, record.Intent.EventID, prior, record.Digest)
		}
		seen[record.Intent.EventID] = record.Digest
